...
function handleRoomUpdate(data) {
  ...
  if (guestUid && !guestReplacedByBot) {
    ...
    if (guestPlayer && !guestPlayer.connected) {
      startDisconnectTimer();
    } else if (guestPlayer && guestPlayer.connected) {
  ...
  // Process guest moves
  if (state && data.moves && data.moveIndex > lastMoveIndex) {
    const newMoves = data.moves.slice(lastMoveIndex);
    lastMoveIndex = data.moveIndex;
...
function processGuestMove(move) {
...
export function hostPlayCard(cardId, chosenColor) {
...
export function hostPassAfterDraw() {
  if (!state || state.gameOver) return;
...
function syncState() {
  ...
  // Guest-visible data: their own hand + host's card count
